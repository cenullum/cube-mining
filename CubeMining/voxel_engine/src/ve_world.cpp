...
float *g_trans_vertex_positions = 0;
float *g_trans_vertex_uvs_base = 0;
float *g_trans_vertex_uvs_local = 0;
float *g_trans_vertex_face_ids = 0;

uint32_t g_result_quad_count = 0;
uint32_t g_result_face_count = 0;
uint32_t g_result_vertex_count = 0;

uint32_t g_trans_result_quad_count = 0;
uint32_t g_trans_result_face_count = 0;
uint32_t g_trans_result_vertex_count = 0;
...
void SetBlock(int x, int y, int z, uint8_t id) {
  if (x < 0 || x >= g_grid_size || y < 0 || y >= g_grid_size || z < 0 ||
      z >= g_grid_size)
    return;
...
bool IsSolid(int x, int y, int z) {
  ...
  if (id == 0)
    return false;
  return g_block_defs[id].registered && (g_block_defs[id].render_type == 0);
}
