...
// ─────────────────────────────────────────────────────────────────────────────
bool CheckCollision(float min_x, float min_y, float min_z, float max_x,
...
// ─────────────────────────────────────────────────────────────────────────────
// TryStepUp: when walking into a wall, try to climb steps up to
// max_step_height. Returns true if the step was successful, and updates pos.
...
static bool TryStepUp(dmVMath::Vector3 &pos, const dmVMath::Vector3 &vel,
                      float half_x, float half_z, float height, float dt_step,
  ...
  for (int i = 1; i <= STEP_SUBSTEPS; i++) {
    ...
    bool lifted_clear = !CheckCollision(
        pos.getX() - half_x, step_y, pos.getZ() - half_z, pos.getX() + half_x,
        step_y + height, pos.getZ() + half_z);
    if (!lifted_clear)
      continue;
...
static void Depenetrate(dmVMath::Vector3 &pos, const dmVMath::Vector3 &size) {
  ...
  const float offsets[4][2] = {
      {0.25f, 0}, {-0.25f, 0}, {0, 0.25f}, {0, -0.25f}};
  for (auto &o : offsets) {
    float tx = pos.getX() + o[0];
    float tz = pos.getZ() + o[1];
    if (!CheckCollision(tx - hx, pos.getY(), tz - hz, tx + hx, pos.getY() + h,
                        tz + hz)) {
      pos.setX(tx);
      pos.setZ(tz);
      return;
...
// ─────────────────────────────────────────────────────────────────────────────
// MoveAndSlide: axis-separated AABB sweep with step-up, snapping &
// depenetration.
//
...
void MoveAndSlide(dmVMath::Vector3 &pos, dmVMath::Vector3 &vel,
  ...
  if (sneaking) {
    ...
    if (currently_grounded) {
      ...
      if (fabsf(dx) > 1e-5f || fabsf(dz) > 1e-5f) {
        ...
        bool combined_safe =
            CheckCollision(nx - hx, pos.getY() - PROBE_DOWN, nz - hz, nx + hx,
                           pos.getY(), nz + hz);

        if (!combined_safe) {
          // Combined failed, try X axis independently
          bool x_safe =
              CheckCollision(nx - hx, pos.getY() - PROBE_DOWN, pos.getZ() - hz,
          ...
          bool z_safe =
              CheckCollision(updated_nx - hx, pos.getY() - PROBE_DOWN, nz - hz,
  ...
  for (int s = 0; s < steps; s++) {
    ...
    if (fabsf(dx) > 1e-7f) {
      float nx = pos.getX() + dx;
      if (CheckCollision(nx - hx, pos.getY(), pos.getZ() - hz, nx + hx,
                         pos.getY() + h, pos.getZ() + hz)) {
    ...
    if (fabsf(dz) > 1e-7f) {
      float nz = pos.getZ() + dz;
      if (CheckCollision(pos.getX() - hx, pos.getY(), nz - hz, pos.getX() + hx,
                         pos.getY() + h, nz + hz)) {
        if (!TryStepUp(pos, vel, hx, hz, h, dt_step, 0.3f)) {
          vel.setZ(0);
        }
    ...
    if (fabsf(dy) > 1e-7f) {
      ...
      if (CheckCollision(pos.getX() - hx, ny, pos.getZ() - hz, pos.getX() + hx,
                         ny + h, pos.getZ() + hz)) {
        if (vel.getY() < 0) {
          is_grounded = true;
          // Snap feet to the exact top of the voxel below us.
          // This eliminates the float drift that slowly pushes us into the
          // floor.
  ...
  if (!is_grounded && vel.getY() <= 0) {
    ...
    if (CheckCollision(pos.getX() - hx, pos.getY() - GROUND_PROBE,
                       pos.getZ() - hz, pos.getX() + hx, pos.getY(),
                       pos.getZ() + hz)) {
      is_grounded = true;
      vel.setY(0);
    }
...
// ─────────────────────────────────────────────────────────────────────────────
// ParticlePhysicsStep: Simulates a particle with bouncing. Point-based
// continuous pseudo-sweep using sub-stepping to correctly resolve corner
// bounces.
// ─────────────────────────────────────────────────────────────────────────────
void ParticlePhysicsStep(dmVMath::Vector3 &pos, dmVMath::Vector3 &vel, float dt,
  ...
  // This prevents the infinite loop bug where a particle enters multiple planes
  // seamlessly
  float speed = dmVMath::Length(vel);
  int steps = (int)ceilf(speed * dt / 0.4f);
  if (steps < 1)
...
bool RayAABBIntersection(const dmVMath::Vector3 &ray_origin,
...
int Lua_Explosion(lua_State *L) {
      ...
      for (int z = min_z; z <= max_z; ++z) {
        ...
        if (dx * dx + dy * dy + dz * dz <= r_sq) {
          ...
          if (b_id != 0) {
            ...
            world_modified = true;
            uint64_t key = ((uint64_t)(x >> 4 & 0xFFFF) << 32) |
                           ((uint64_t)(y >> 4 & 0xFFFF) << 16) |
                           (uint64_t)(z >> 4 & 0xFFFF);
  ...
  for (auto &npc : g_npcs) {
    ...
    if (dmVMath::Length(to_npc) <= radius + 1.0f) {
      ...
      dmVMath::Vector3 kb = dmVMath::Normalize(dmVMath::Vector3(
                                to_npc.getX(), 0, to_npc.getZ())) *
                                (damage * 0.3f) +
                            dmVMath::Vector3(0, 10.0f, 0);
      npc.health -= damage;
      npc.vel = npc.vel + kb;
      ...
      dmMessage::Post(0, &receiver, dmHashString64("damaged"), 0, 0, 0, 0, 0,
                      0);
      lua_newtable(L);
      dmScript::PushHash(L, npc.id);
      lua_setfield(L, -2, "id");
      lua_pushnumber(L, damage);
      lua_setfield(L, -2, "damage");
      dmScript::PushVector3(L, kb);
      lua_setfield(L, -2, "kb_dir");
  ...
  for (uint64_t key : touched_chunks) {
    lua_newtable(L);
    lua_pushinteger(L, (int)(key >> 32 & 0xFFFF));
    lua_setfield(L, -2, "x");
    lua_pushinteger(L, (int)(key >> 16 & 0xFFFF));
    lua_setfield(L, -2, "y");
    lua_pushinteger(L, (int)(key & 0xFFFF));
    lua_setfield(L, -2, "z");
...
int Lua_ShootRay(lua_State *L) {
  ...
  dmVMath::Vector3 step(dir.getX() > 0 ? 1 : -1, dir.getY() > 0 ? 1 : -1,
                        dir.getZ() > 0 ? 1 : -1);
  dmVMath::Vector3 delta(fabsf(1.0f / (dir.getX() + 1e-9f)),
                         fabsf(1.0f / (dir.getY() + 1e-9f)),
                         fabsf(1.0f / (dir.getZ() + 1e-9f)));
  int ix = (int)floorf(ray_grid.getX() + 0.5f),
      iy = (int)floorf(ray_grid.getY() + 0.5f),
      iz = (int)floorf(ray_grid.getZ() + 0.5f);
  dmVMath::Vector3 next_t(
      dir.getX() > 0 ? (ix + 0.5f - ray_grid.getX()) * delta.getX()
                     : (ray_grid.getX() - (ix - 0.5f)) * delta.getX(),
      dir.getY() > 0 ? (iy + 0.5f - ray_grid.getY()) * delta.getY()
                     : (ray_grid.getY() - (iy - 0.5f)) * delta.getY(),
      dir.getZ() > 0 ? (iz + 0.5f - ray_grid.getZ()) * delta.getZ()
                     : (ray_grid.getZ() - (iz - 0.5f)) * delta.getZ());

  float dist = 0;
  dmVMath::Vector3 norm(0, 0, 0);
  while (dist < max_dist) {
    if (GetBlock(ix, iy, iz) != 0) {
      hits.push_back({dist, 0, true,
                      dmVMath::Vector3((float)ix, (float)iy, (float)iz), norm});
      break;
    }
  ...
  for (const auto &npc : g_npcs) {
    ...
    if (RayAABBIntersection(
            origin, dir,
            npc.pos + dmVMath::Vector3(-half.getX(), 0, -half.getZ()),
            npc.pos +
                dmVMath::Vector3(half.getX(), npc.size.getY(), half.getZ()),
            t_hit)) {
      if (t_hit < max_dist) {
        bool obscured = false;
        for (auto &h : hits)
          if (h.is_block && h.dist < t_hit)
            obscured = true;
        if (!obscured) {
          hits.push_back(
              {t_hit, npc.id, false, npc.pos, dmVMath::Vector3(0, 0, 0)});
          if (damage > 0) {
            for (auto &n : g_npcs)
              if (n.id == npc.id) {
                n.health -= damage;
                ...
                dmMessage::URL rec;
                dmMessage::ResetURL(&rec);
                rec.m_Socket = n.socket;
                rec.m_Path = dmGameObject::GetIdentifier(n.instance);
                dmMessage::Post(0, &rec, dmHashString64("damaged"), 0, 0, 0, 0,
                                0, 0);
                break;
              }
          }
  ...
  for (auto &h : hits) {
    lua_newtable(L);
    lua_pushboolean(L, h.is_block);
    lua_setfield(L, -2, "is_block");
    lua_pushnumber(L, h.dist);
    lua_setfield(L, -2, "dist");
    dmScript::PushVector3(L, h.pos);
    lua_setfield(L, -2, "pos");
    dmScript::PushVector3(L, h.normal);
    lua_setfield(L, -2, "normal");
    if (!h.is_block) {
      dmScript::PushHash(L, h.npc_id);
      lua_setfield(L, -2, "id");
    }
    lua_rawseti(L, -2, ++hcount);
    if (h.is_block || hcount > penetration)
      break;
  }
  return 1;
}
...
int Lua_MoveAndSlide(lua_State *L) {
  dmVMath::Vector3 pos = *dmScript::ToVector3(L, 1),
                   vel = *dmScript::ToVector3(L, 2),
                   size = *dmScript::ToVector3(L, 3);
...
int Lua_CheckCollision(lua_State *L) {
  float x1 = luaL_checknumber(L, 1), y1 = luaL_checknumber(L, 2),
        z1 = luaL_checknumber(L, 3), x2 = luaL_checknumber(L, 4),
        y2 = luaL_checknumber(L, 5), z2 = luaL_checknumber(L, 6);
